...
def process_directory(directory, base_dir, result_base_dir, processed_dirs):
    # Check if the directory has already been processed to avoid redundancy
    ...
    records_file_path = os.path.join(records_dir, f"{records_filename}.txt")
    ...
    if os.path.exists(records_file_path):
        ...
        with open(records_file_path, 'r') as records_file:
            for line in records_file:
                ...
                mat_file_path = os.path.join(base_dir, directory, f"{file_name}.mat").replace('/', os.sep)
    ...
    # Add the current directory to the list of processed directories
    processed_dirs.add(directory)

    # Recursively process subdirectories if the records directory exists
    if os.path.exists(records_dir):
        for subdir in os.listdir(records_dir):
            subdir_path = os.path.join(records_dir, subdir)
            if os.path.isdir(subdir_path):
                process_directory(subdir, base_dir, result_base_dir, processed_dirs)
...
result_directory = 'processed data'
...
# Set to keep track of processed directories to avoid redundancy
processed_dirs = set()

# Iterate over each subfolder path from the first RECORDS file
for subfolder_path in subfolder_paths:
    process_directory(subfolder_path, records_directory, result_directory, processed_dirs)
